...
-opaque queue(EVL) :: {queue, list(EVL), list(EVL)}.

-spec new() -> queue(any()).
new() ->
    {queue, [], []}.

-spec from_list(list(EVO)) -> queue(EVO).
from_list(List) ->
    {queue, [], List}.

-spec to_list(queue(EVR)) -> list(EVR).
to_list(Queue) ->
...
-spec pop_front(queue(EWJ)) -> {ok, {EWJ, queue(EWJ)}} | {error, nil}.
pop_front(Queue) ->
    case Queue of
        {queue, [], []} ->
            {error, nil};
...
-spec reverse(queue(EWO)) -> queue(EWO).
reverse(Queue) ->
...
-spec is_logically_equal(queue(EWR), queue(EWR), fun((EWR, EWR) -> boolean())) -> boolean().
...
-spec is_equal(queue(EWZ), queue(EWZ)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),

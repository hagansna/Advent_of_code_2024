...
-spec equal(FRT, FRT) -> nil.
equal(A, B) ->
    gleeunit_ffi:should_equal(A, B).

-spec not_equal(FRU, FRU) -> nil.
not_equal(A, B) ->
    gleeunit_ffi:should_not_equal(A, B).

-spec be_ok({ok, FRV} | {error, any()}) -> FRV.
be_ok(A) ->
    gleeunit_ffi:should_be_ok(A).

-spec be_error({ok, any()} | {error, FSA}) -> FSA.
be_error(A) ->
    gleeunit_ffi:should_be_error(A).

-spec be_some(gleam@option:option(FSD)) -> FSD.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
